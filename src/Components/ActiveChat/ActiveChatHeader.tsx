...
export const ActiveChatHeader = (props: any) => {
    ...
    useEffect(() => {
        if (searchText != '') {
            setFindedUsers([...activeChat.users.filter(x => x.username?.includes(searchText))])
        } else {
        ...
        if (isAdmin) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    socket.emit('removeUser', { roomId: activeChat.id, username: username })
                    Swal.fire(
                        language.words?.Deleted
                    )
                } else if (result.dismiss === Swal.DismissReason.cancel) {
                    Swal.fire(
                        language.words?.Canceled,
                    )
                }
            })
        }
            ...
            <Offcanvas show={show} placement="end" onHide={handleClose} {...props}>
                <Offcanvas.Header className="d-flex flex-column align-items-start chat-info-header">
                    ...
                    <div>
                        ...
                        <div className="d-flex align-items-center" style={{ cursor: 'pointer' }} onClick={onCopyClick}>
                        ...
                        if (activeChat.admins.find(x => x.email == user.email)) {
                            ...
                            return (
                                <div className="d-flex">
                                    ...
                                    {
                                        isAdmin ?
                                            <div className="ms-2" style={{ cursor: 'pointer' }} onClick={(e) => deleteUser(user.username)}>
