...
export const ActiveChatHeader = (props: any) => {
  ...
  const deleteRoom = (e: any) => {
    socket.send("DeleteRoom", activeChat)
  }

  const exitRoom = (e: any) => {
    socket.send("ExitRoom", activeChat)
  }
    ...
    <>
      ...
      <Offcanvas show={show} placement="end" onHide={handleClose} {...props}>
        <Offcanvas.Header className="d-flex flex-column align-items-start chat-info-header">
          ...
          {activeChat.admins.find((x) => x.email == user.email) && (
              <div style={{ cursor: "pointer" }} onClick={deleteRoom}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
        ...
        <Offcanvas.Body className="chat-info-body d-flex flex-column">
          ...
          <div style={{cursor: 'pointer'}} onClick={exitRoom}>Выйти из комнаты</div>
        </Offcanvas.Body>
      </Offcanvas>
    </>
  );
