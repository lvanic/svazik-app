...
export const Message = (props: any) => {
  ...
  const onMessageContextClick = (e: any) => {
    ...
    if (props.name == user.username) {
      ...
      activeChat.admins.findIndex((x) => x.username == user.username) != -1
    ) {
      setDropDownCoords({ X: e.clientX, Y: e.clientY });
      setIsAdmin(true);
      setShowContextMenu(true);
  ...
  return (
    ...
    <div key={props.id}>
      <div
        onContextMenu={onMessageContextClick}
        ref={messageRef}
        className={`message-field d-flex align-items-center justify-content-${props.location}`}
      >
        <Card
          className={`ps-${
            5 - position
          } pe-${position} pt-2 pb-2 mb-1 d-flex align-items-${
            props.location
      ...
      {showContextMenu && (
        ...
        >
          {isAdmin ? null : (
            <Dropdown.Item
              onClick={(e: any) => editMessage(props.id)}
              className="d-flex justify-content-around context-menu-item"
              name="Edit"
            >
              <div>
                <svg
          ...
          <Dropdown.Item
            onClick={(e: any) => screamMessage(props.id)}
            className="d-flex justify-content-around context-menu-item"
            name="Scream"
          >
            <div>
          ...
          <Dropdown.Item
            onClick={(e: any) => deleteMessage(props.id)}
            className="d-flex justify-content-around context-menu-item"
            name="Delete"
          >
            <div>
